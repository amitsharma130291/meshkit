/**
 * Orchestration for /3mf-to-stl/. Follows the same pattern as
 ...
 * piece. The differences here: the worker returns both a preview geometry
 * AND a finished STL buffer, the info panel shows conversion-specific
 * fields (source unit, object/build-item/component-instance counts,
 * warnings) instead of a user-selectable unit, and the primary output
 * action is a file download rather than a screenshot.
 *
...
import type { ConversionWarning, ThreeMFToSTLResult, ThreeMFUnit } from "../../lib/threemf/types";
...
const STAGE_LABELS: Record<string, string> = {
  ...
  "locating-model": "Locating 3D model…",
...
function init(): void {
  ...
  const infoSourceUnit = document.querySelector<HTMLElement>("[data-info-source-unit]")!;
  const infoTriangles = document.querySelector<HTMLElement>("[data-info-triangles]")!;
  const infoObjectCount = document.querySelector<HTMLElement>("[data-info-object-count]")!;
  const infoBuildItems = document.querySelector<HTMLElement>("[data-info-build-items]")!;
  const infoComponentInstances = document.querySelector<HTMLElement>("[data-info-component-instances]")!;
  ...
  if (batchWorkspaceRoot) initBatchEntitlementGate(batchWorkspaceRoot, "convert-3mf-to-stl", "batch-conversion", createProductionEntitlementProvider());
  ...
  function buildMeshFromResult(result: ThreeMFToSTLResult): void {
  ...
  function renderInfo(result: ThreeMFToSTLResult): void {
    infoFilename.textContent = lastFileName;
    infoFilesize.textContent = formatFileSize(lastFileSize);
    infoSourceUnit.textContent = UNIT_LABELS[result.sourceUnit];
    infoTriangles.textContent = formatTriangleCount(result.triangleCount);
    infoObjectCount.textContent = String(result.objectCount);
    infoBuildItems.textContent = String(result.buildItemCount);
    infoComponentInstances.textContent = String(result.componentInstanceCount);
  ...
  async function renderResult(result: ThreeMFToSTLResult): Promise<void> {
    try {
      await ensureViewer();
      buildMeshFromResult(result);

      lastStlBuffer = result.stlBuffer;
      lastDownloadFilename = buildConvertedFilename(lastFileName, "3mf", "stl");
  ...
  function createThreeMFWorker(): WorkerLike {
    const worker = new Worker(new URL("../../workers/threemf-to-stl.worker.ts", import.meta.url), { type: "module" });
  ...
  async function initWorker(): Promise<void> {
    workerReady = false;
    workerClient = new WorkerClient(createThreeMFWorker, {
      onReady: () => {
        workerReady = true;
      ...
      onResult: (result) => {
        ...
        void renderResult(result as ThreeMFToSTLResult);
      },
      onError: (error) => {
        cancelButton.hidden = true;
        disposeStlOutput();
