/**
 * Orchestration for /stl-to-3mf/. Follows the same pattern as every other
...
import type { ConversionWarning, STLToThreeMFResult } from "../../lib/stl-to-threemf/types";
...
const STAGE_LABELS: Record<string, string> = {
  ...
  "writing-model-xml": "Writing model…",
  "writing-package-metadata": "Preparing package…",
...
function init(): void {
  ...
  const infoOutputTriangles = document.querySelector<HTMLElement>("[data-info-output-triangles]")!;
  ...
  let lastThreeMFBuffer: ArrayBuffer | null = null;
  let lastDownloadFilename = "model.3mf";
  ...
  function disposeThreeMFOutput(): void {
    lastThreeMFBuffer = null;
  ...
  function buildMeshFromResult(result: STLToThreeMFResult): void {
    ...
    // Flat shading by default: an STL/3MF pair never carries materials or
  ...
  function renderInfo(result: STLToThreeMFResult): void {
    ...
    infoOutputTriangles.textContent = formatTriangleCount(result.outputTriangleCount);
  ...
  async function renderResult(result: STLToThreeMFResult): Promise<void> {
    try {
      await ensureViewer();
      buildMeshFromResult(result);

      lastThreeMFBuffer = result.threeMFBuffer;
      lastDownloadFilename = buildConvertedFilename(lastFileName, "stl", "3mf");
  ...
  function createSTLToThreeMFWorker(): WorkerLike {
    const worker = new Worker(new URL("../../workers/stl-to-threemf.worker.ts", import.meta.url), { type: "module" });
  ...
  async function initWorker(): Promise<void> {
    workerReady = false;
    workerClient = new WorkerClient(createSTLToThreeMFWorker, {
      onReady: () => {
        workerReady = true;
      ...
      onResult: (result) => {
        ...
        void renderResult(result as STLToThreeMFResult);
      },
      onError: (error) => {
        cancelButton.hidden = true;
        disposeThreeMFOutput();
        showError(error);
        setState("error");
      },
      onCancelled: () => {
  ...
  function downloadThreeMF(): void {
    if (!lastThreeMFBuffer) return;
    try {
      const blob = new Blob([lastThreeMFBuffer], { type: "model/3mf" });
  ...
  downloadButton.addEventListener("click", downloadThreeMF);
