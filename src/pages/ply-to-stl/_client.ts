/**
 * Orchestration for /ply-to-stl/. Follows the same pattern as
 * src/pages/obj-to-stl/_client.ts (lazy Three.js viewport, camera fit with
 * OrbitControls damping cleared before repositioning, worker lifecycle,
 * no source-unit field since PLY — like OBJ — has no unit system) — see
 * that file and docs/ARCHITECTURE.md for the reasoning behind each piece.
 * The differences here: the info panel shows PLY-specific counts (source
 * vertices/faces, declared vertex/face property counts, skipped unknown
 * elements/properties, encoding) instead of OBJ's object/group counts.
 *
...
import type { ConversionWarning, PLYToSTLResult } from "../../lib/ply/types";
...
function init(): void {
  ...
  const infoSkippedUnknown = document.querySelector<HTMLElement>("[data-info-skipped-unknown]")!;
  ...
  if (batchWorkspaceRoot) initBatchEntitlementGate(batchWorkspaceRoot, "convert-ply-to-stl", "batch-conversion", createProductionEntitlementProvider());
  ...
  function buildMeshFromResult(result: PLYToSTLResult): void {
    ...
    // Flat shading by default: PLY vertex normals and colors aren't
  ...
  function renderInfo(result: PLYToSTLResult): void {
    infoFilename.textContent = lastFileName;
    infoFilesize.textContent = formatFileSize(lastFileSize);
    infoFormat.textContent = FORMAT_LABELS[result.format] ?? result.format;
    ...
    infoSkippedUnknown.textContent = formatTriangleCount(result.unknownElementCount + result.unknownPropertyCount);
  ...
  async function renderResult(result: PLYToSTLResult): Promise<void> {
    try {
      await ensureViewer();
      buildMeshFromResult(result);

      lastStlBuffer = result.stlBuffer;
      lastDownloadFilename = buildConvertedFilename(lastFileName, "ply", "stl");
  ...
  function createPLYWorker(): WorkerLike {
    const worker = new Worker(new URL("../../workers/ply-to-stl.worker.ts", import.meta.url), { type: "module" });
  ...
  async function initWorker(): Promise<void> {
    workerReady = false;
    workerClient = new WorkerClient(createPLYWorker, {
      onReady: () => {
        workerReady = true;
      ...
      onResult: (result) => {
        ...
        void renderResult(result as PLYToSTLResult);
      },
      onError: (error) => {
        cancelButton.hidden = true;
        disposeStlOutput();
