/**
 * Orchestration for /glb-to-stl/. Follows the same pattern as
 * src/pages/obj-to-stl/_client.ts and src/pages/3mf-to-stl/_client.ts
 * (lazy Three.js viewport, camera fit with OrbitControls damping cleared
 * before repositioning, worker lifecycle) — see those files and
 * docs/ARCHITECTURE.md for the reasoning behind each piece. The
 * differences here: the info panel shows GLB-specific counts (scene name,
 * mesh/primitive/node-instance counts, skipped-degenerate count) and
 * dimensions are always millimetres (GLB has a real source unit — metres —
 * unlike OBJ's unitless coordinates), and the preview material uses flat
 * shading since GLB materials/animations/skins aren't preserved in STL.
 *
...
import { formatFileSize, formatDimensionWithUnit, formatTriangleCount } from "../../lib/stl/format";
import type { ConversionWarning, GLBToSTLResult } from "../../lib/glb/types";
...
function init(): void {
  ...
  const infoScene = document.querySelector<HTMLElement>("[data-info-scene]")!;
  ...
  if (batchWorkspaceRoot) initBatchEntitlementGate(batchWorkspaceRoot, "convert-glb-to-stl", "batch-conversion", createProductionEntitlementProvider());
  ...
  let lastDownloadFilename = "model.stl";
  ...
  function disposeStlOutput(): void {
    lastStlBuffer = null;
  ...
  function buildMeshFromResult(result: GLBToSTLResult): void {
    ...
    // Flat shading by default: GLB materials/animations/skins aren't
    // preserved in the STL output, so the preview shouldn't imply they are.
  ...
  function renderWarnings(warnings: ConversionWarning[]): void {
    ...
    heading.textContent = "STL stores surface geometry only. Some things in this file won't carry over:";
  ...
  function renderBounds(bounds: STLBounds): void {
    infoWidth.textContent = formatDimensionWithUnit(bounds.size[0], "mm");
    infoHeight.textContent = formatDimensionWithUnit(bounds.size[1], "mm");
    infoDepth.textContent = formatDimensionWithUnit(bounds.size[2], "mm");
  }

  async function renderResult(result: GLBToSTLResult): Promise<void> {
    try {
      await ensureViewer();
      buildMeshFromResult(result);

      lastStlBuffer = result.stlBuffer;
      lastDownloadFilename = buildConvertedFilename(lastFileName, "glb", "stl");
  ...
  function createGLBWorker(): WorkerLike {
    const worker = new Worker(new URL("../../workers/glb-to-stl.worker.ts", import.meta.url), { type: "module" });
  ...
  async function initWorker(): Promise<void> {
    workerReady = false;
    workerClient = new WorkerClient(createGLBWorker, {
      onReady: () => {
        workerReady = true;
      ...
      onResult: (result) => {
        ...
        void renderResult(result as GLBToSTLResult);
      },
      onError: (error) => {
        cancelButton.hidden = true;
        disposeStlOutput();
  ...
  function downloadStl(): void {
    ...
    try {
      const blob = new Blob([lastStlBuffer], { type: "model/stl" });
