/**
 * Orchestration for /obj-to-stl/. Follows the same pattern as
 * src/pages/3mf-to-stl/_client.ts (lazy Three.js viewport, camera fit with
 ...
 * piece. The differences here: there's no source-unit field (OBJ has no
 * unit system — dimensions are shown as plain model-unit numbers), the
 * info panel shows OBJ-specific counts (source vertices/faces, objects,
 * groups) instead of 3MF's component/build-item counts, and the preview
 * material uses flat shading by default since OBJ smoothing groups and
 * materials aren't preserved in the STL output.
 *
...
import type { ConversionWarning, OBJToSTLResult } from "../../lib/obj/types";
...
function init(): void {
  ...
  const infoGroupCount = document.querySelector<HTMLElement>("[data-info-group-count]")!;
  ...
  function buildMeshFromResult(result: OBJToSTLResult): void {
    ...
    // Flat shading by default: OBJ smoothing groups and materials aren't
  ...
  function renderInfo(result: OBJToSTLResult): void {
    ...
    infoGroupCount.textContent = String(result.groupCount);
  ...
  async function renderResult(result: OBJToSTLResult): Promise<void> {
    try {
      await ensureViewer();
      buildMeshFromResult(result);

      lastStlBuffer = result.stlBuffer;
      lastDownloadFilename = buildConvertedFilename(lastFileName, "obj", "stl");
  ...
  function createOBJWorker(): WorkerLike {
    const worker = new Worker(new URL("../../workers/obj-to-stl.worker.ts", import.meta.url), { type: "module" });
  ...
  async function initWorker(): Promise<void> {
    workerReady = false;
    workerClient = new WorkerClient(createOBJWorker, {
      onReady: () => {
        workerReady = true;
      ...
      onResult: (result) => {
        ...
        void renderResult(result as OBJToSTLResult);
      },
      onError: (error) => {
        cancelButton.hidden = true;
        disposeStlOutput();
